...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($jkp['jak_signup_firstname'])) {
        $errors['e1'] = $jkl['ef'];
    }
    if (empty($jkp['jak_signup_secondname'])) {
        $errors['e1'] = $jkl['es'];
    }

    if ($jkp['jak_signup_email'] == '' || !filter_var($jkp['jak_signup_email'], FILTER_VALIDATE_EMAIL)) {
        $errors['e2'] = $jkl['e3'];
    }

    if (jak_field_not_exist(strtolower($jkp['jak_signup_email']), 'user', "email")) {
        $errors['e2'] = $jkl['e18'];
    }
 
    // if (jak_field_not_exist(strtolower($jkp['jak_signup_username']), 'user', "username")) {
    //     $errors['e4'] = $jkl['e9'];
    // }
    $userlen = $jakdb->select("user", "*");
    $len = sizeof($userlen);


    if ($jkp['jak_signup_password'] != $jkp['jak_signup_confirm_password']) {
        $errors['e5'] = $jkl['e10'];
    } elseif (strlen($jkp['jak_signup_password']) <= '7') {
        $errors['e6'] = $jkl['e11'];
    } else {
        $updatepass = true;
    }
    ...
    if (count($errors) == 0) {
        ...
        $jakdb->insert('user', [
            "departments" => "0",
            "password" => hash_hmac('sha256', $jkp['jak_signup_password'], DB_PASS_HASH),
            "username" => "customer" . "$len",
            "name" => trim($jkp['jak_signup_firstname']  . " " . $jkp['jak_signup_secondname']),
            "email" => filter_var($jkp['jak_signup_email'], FILTER_SANITIZE_EMAIL),
            "responses" => "1",
            "files" => "1",
            "operatorchat" => "1",
            ...
            "language" => "en",
            "invitationmsg" => "Hello",
            "permissions" => "widget,settings,leads",
            "access" => "1",
            "time" => $jakdb->raw("NOW()")
        ]);
